...
public interface SolutionRepository extends CrudRepository<Solution,Long> {
    ...
    @Query("select count(distinct s.problem) from Solution s where s.resolvedProblem=?1 and s.creator=?2")
    ...
    @Query("select s.problem from Solution s where s.creator=?2 and s.resolvedProblem=?1")
